...
class ClassroomManagementAdapterTest {
    ...
    void enrichesMaterialsByRoleWithMaterialDetails() {
        ClassroomRepositoryPort classroomRepository =
                mock(ClassroomRepositoryPort.class);

        MembershipRepositoryPort memberRepository =
                mock(MembershipRepositoryPort.class);

        JoinCodeGenerator joinCodeGenerator =
                mock(JoinCodeGenerator.class);

        MemberRoleServiceAdapter memberRoleService =
                mock(MemberRoleServiceAdapter.class);

        MaterialDetailsRepositoryPort materialDetailsRepository =
                mock(MaterialDetailsRepositoryPort.class);

        MaterialDetailsRequestPublisherPort materialDetailsRequestPublisher =
                mock(MaterialDetailsRequestPublisherPort.class);

        MemberProfileRepositoryPort memberProfileRepository =
                mock(MemberProfileRepositoryPort.class);

        UserProfileDetailsRequestPublisherPort userProfileDetailsRequestPublisher =
                mock(UserProfileDetailsRequestPublisherPort.class);

        ClassroomManagementAdapter adapter =
                new ClassroomManagementAdapter(
                        classroomRepository,
                        memberRepository,
                        joinCodeGenerator,
                        memberRoleService,
        ...
        when(classroomRepository.findById(7L))
        ...
        List<MaterialReferenceWithDetailsDto> result =
                adapter.getClassroomMaterialsByRole(
                        7L,
                        ClassroomRole.STUDENT
                );
        ...
        MaterialReferenceWithDetailsDto dto =
                result.getFirst();
        ...
        verify(classroomRepository)
                .findById(7L);

        verify(materialDetailsRepository)
                .findByMaterialId(101L);
    }
}